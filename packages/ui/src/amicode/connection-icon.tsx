...
export function ConnectionIcon(props: { conn: ConnectionView }) {
  ...
  return (
    <div class="relative shrink-0" style={{ width: "18px", height: "18px" }}>
      <div
        class="w-[18px] h-[18px] rounded-[4px] flex items-center justify-center overflow-hidden bg-surface-raised-base border border-border-weak-base"
        style={{ "font-size": "10px", "font-weight": "600" }}
      >
        <Show when={isSvg()} fallback={<span class="text-text-base">{letter()}</span>}>
          <span class="w-[14px] h-[14px] flex items-center justify-center text-text-base" innerHTML={props.conn.icon ?? ""} />
        </Show>
      </div>
