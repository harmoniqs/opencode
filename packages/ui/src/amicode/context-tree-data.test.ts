...
const turn = (id: string, title: string, refs: ContextTurn["refs"], busy = false): ContextTurn => ({
  id,
  title,
  refs,
  busy,
...
describe("buildContextTree", () => {
  test("root → turns → leaves; roman numerals + prompt excerpts", () => {
    const tree = buildContextTree([
      turn("m1", "optimize a CZ gate", [
        { label: "setup.jl", type: "package", path: "/p/setup.jl" },
        { label: "transmon", type: "skill" },
      ]),
      turn("m2", "now fix stagnation", [{ label: "Explore", type: "agent" }]),
    ])
    expect(tree.kind).toBe("root")
    expect(tree.children!.map((t) => t.label)).toEqual(["I · optimize a CZ gate", "II · now fix stagnation"])
    expect(tree.children![0].children!.length).toBe(2)
  })
  test("consider refs (searches) never enter the tree", () => {
    const tree = buildContextTree([turn("m1", "hunt", [{ label: "saveat", type: "resource", consider: true }])])
    expect(tree.children![0].children!.length).toBe(0)
  })
  test("a re-touched file dedups into a recall link, not a duplicate", () => {
    const tree = buildContextTree([
      turn("m1", "read it", [{ label: "solve.jl", type: "package", path: "/p/solve.jl" }]),
      turn("m2", "read it again", [{ label: "solve.jl", type: "package", path: "/p/solve.jl" }]),
    ])
  ...
  test("vault paths are flagged for the Vault panel", () => {
    const tree = buildContextTree([
      turn("m1", "vault read", [{ label: "STRATEGY.md", type: "note", path: "/u/.amico/vaults/armonissima/STRATEGY.md" }]),
    ])
    expect(tree.children![0].children![0].vault).toBe(true)
  })
  test("marathon sessions fold old turns into one earlier branch", () => {
    const turns = Array.from({ length: 30 }, (_, i) =>
      turn(`m${i}`, `turn ${i}`, [{ label: `f${i}.md`, type: "note", path: `/p/f${i}.md` }]),
    )
    ...
    const again = buildContextTree([...turns, turn("m30", "again", [{ label: "f0.md", type: "note", path: "/p/f0.md" }])])
