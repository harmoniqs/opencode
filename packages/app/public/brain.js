...
/* The demo chrome ships inert in <template id="demo-chrome"> — embeds never
   have it in the document at all. Stamp it here, BEFORE any chrome bindings
   below run: standalone gets everything, a loader embed gets the mast only,
   live/inline get nothing. */
(function stampDemoChrome() {
  var tpl = document.getElementById("demo-chrome");
  if (!tpl) return;
  var mode = new URLSearchParams(location.search).get("mode") || window.__AMICO_BRAIN_MODE__ || "";
  if (mode === "live" || mode === "inline") return;
  if (mode === "loader") {
    for (var id of ["legend", "controls", "colophon"]) {
      var n = tpl.content.getElementById(id);
      if (n) n.remove();
    }
  }
...
function setStatus(text, instant) {
  ...
  if (!el) return; // embeds carry no statusline
...
function setTempoMark(txt) { const el = document.getElementById("tempo-mark"); if (el) el.innerHTML = txt; }
      ...
      if (con.progress < 1) at(0.05, tickChart);
        ...
        const drainTick = () => {
          ...
          if (player.drain > 0) at(0.08, drainTick);
            ...
            if (autoBox && autoBox.checked) at(6, autoAdvance);
          }
        };
        drainTick();
      }
...
function autoAdvance() {
  const autoBox = document.getElementById("auto");
  if (isLiveEmbed || player.running || !autoBox || !autoBox.checked) return;
...
function draw(nowMs) {
  ...
  if (caret && !reduceMotion) caret.classList.toggle("off", Math.floor(clock.beat * 2) % 2 === 1);
...
/* ---------- controls (standalone only — embeds never stamp the chrome) ---------- */
...
if (legend) for (const cat of ["knowledge", "skills", "results", "code", "agents", "thought"]) {
...
if (!isLiveEmbed) setTimeout(() => {
  if (selScenario) selScenario.value = DATA.traces[0].id;
