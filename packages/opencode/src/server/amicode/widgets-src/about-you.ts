  ...
  mount: function (el, amico) {
    ...
    var render = function () {
      ...
      var avatar = you.avatar
        ? '<img src="' + esc(you.avatar) + '" alt="" onerror="this.style.display=\\'none\\'" style="width:44px;height:44px;border-radius:10px;object-fit:cover;flex-shrink:0">'
        : '<div style="width:44px;height:44px;flex-shrink:0;border-radius:10px;background:var(--amc-layer2);color:var(--amc-text-muted);display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:600">' + esc(initials(you.name)) + '</div>'
      ...
      var editForm =
        ...
        '<input data-f-name placeholder="name" value="' + esc(draft.name) + '" style="font-size:12px;padding:4px 8px;border:1px solid var(--amc-border);border-radius:6px;background:var(--amc-bg);color:var(--amc-text)">' +
        '<div style="position:relative">' +
        '<input data-f-affiliation placeholder="affiliation" value="' + esc(draft.affiliation) + '" autocomplete="off" style="width:100%;box-sizing:border-box;font-size:12px;padding:4px 8px;border:1px solid var(--amc-border);border-radius:6px;background:var(--amc-bg);color:var(--amc-text)">' +
        (suggestions.length
          ? '<div style="position:absolute;left:0;right:0;top:100%;z-index:5;background:var(--amc-layer2);border:1px solid var(--amc-border);border-radius:6px;overflow:hidden">' +
          ...
          : '') +
        '</div>' +
        '<input data-f-focus placeholder="focus (e.g. transmon pulse design)" value="' + esc(draft.focus) + '" style="font-size:12px;padding:4px 8px;border:1px solid var(--amc-border);border-radius:6px;background:var(--amc-bg);color:var(--amc-text)">' +
        '<input data-f-scholar placeholder="Google Scholar URL" value="' + esc(draft.scholar) + '" style="font-size:12px;padding:4px 8px;border:1px solid var(--amc-border);border-radius:6px;background:var(--amc-bg);color:var(--amc-text)">' +
        '<div style="display:flex;gap:8px">' +
        '<button type="button" data-save ' + (saving ? 'disabled' : '') + ' style="font-size:12px;padding:4px 12px;border:1px solid var(--amc-accent);border-radius:6px;background:var(--amc-layer2);color:var(--amc-text);cursor:pointer">' + (saving ? 'Saving\\u2026' : 'Save') + '</button>' +
        '<button type="button" data-cancel style="font-size:12px;padding:4px 12px;border:1px solid var(--amc-border);border-radius:6px;background:transparent;color:var(--amc-text-muted);cursor:pointer">Cancel</button>' +
        '</div></div>'
      ...
      if (fresh && !editing) {
        body =
          ...
          '<button type="button" data-firstrun style="align-self:flex-start;border:1px solid var(--amc-accent);border-radius:6px;background:var(--amc-layer2);color:var(--amc-text);padding:5px 12px;font-size:12px;cursor:pointer">Get started &#8594;</button>'
      } else if (!editing) {
