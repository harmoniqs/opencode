...
export default {
  mount: function (el, amico) {
    ...
    var render = function () {
      ...
      el.innerHTML =
        '<div style="display:flex;flex-direction:column;gap:6px;min-width:0;min-height:88px;border:1px solid var(--amc-border);border-radius:10px;background:var(--amc-layer);padding:var(--amc-pad-tile)">' +
        ...
        '<button type="button" data-upload ' + (busy ? 'disabled' : '') + ' style="font-size:11px;padding:3px 10px;border:1px solid var(--amc-accent);border-radius:6px;background:var(--amc-layer2);color:var(--amc-text);cursor:pointer">' + (busy ? 'Uploading\\u2026' : 'Upload PDF') + '</button>' +
