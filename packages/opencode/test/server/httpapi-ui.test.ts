...
describe("HttpApi UI fallback", () => {
  ...
  // Regression (native brain, 2026-07-20): the "amico is thinking" animation
  // now renders on an in-document canvas (packages/ui amicode/brain-engine),
  // so /brain.html and /brain.js no longer exist and their auth exemptions
  // are gone. The public surface must not silently regrow: with a password
  // armed, the old brain paths are authed like any other route.
  it.live("no longer exempts the retired brain iframe paths from auth", () =>
    Effect.gen(function* () {
